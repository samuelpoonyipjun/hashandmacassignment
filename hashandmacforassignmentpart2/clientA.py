    ...
    while True:
        try:
            ...
            if verify_signature(other_client_public_key, decrypted_message, received_signature):
                print(f"\nClient B: {decrypted_message}")
                print("Input message (Type exit to close connection): ", end="", flush=True) # keeps prompting user to input message
            else:
...
def start_clientA(host='127.0.0.1', port=65432):
    # Generate RSA keys
